/**
 * 此插件是由xgplayer-mp4@1.1.8迁移过来
 */

import "core-js/modules/es6.promise.js";
import "core-js/modules/es7.string.pad-start";
import Player from "./../../../src/video";
import MP4 from "./mp4";
  ...
  let url = player.config.url;
    ...
    let loadData = (i = 0, time = player.currentTime) => {
        ...
        player.timer = setTimeout(function () {
            player.mp4.seek(time + i * 0.1).then(
                (buffer) => {
                    if (buffer) {
                        let mse = player.mse;
                        mse.updating = true;
    ...
    let init = (url) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            mp4.on("error", (e) => {
                console.log("mp4errpr", e);
                reject(e);
            });
        });
    };
    if (
        // ["chrome", "firfox", "safari"].some(
        //     (item) => item === util.browser
        // ) &&
        ...
        player.start = function (url = mainURL) {
            ...
            player.once("canplay", () => {
                ...
                if (util.browser === "safari" && player.buffered) {
        ...
        player.mp4ProgressTimer = setInterval(
            timeupdateFunc,
            player.config.mp4ProgressTimer || 300
        );
        ...
        let waitingFunc = function () {
            ...
            if (
                duration - player.currentTime < 0.5 &&
                duration - range[1] < 0.5
            ) {
                player.mse.endOfStream();
            } else {
