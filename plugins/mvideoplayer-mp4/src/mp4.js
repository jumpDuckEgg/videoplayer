...
class MP4 extends EventEmitter {
    ...
    moovParse() {
        ...
        let sps, pps, profile, width, height;
        ...
        traks.forEach((trak) => {
            ...
            if (hdlr.handleType === "vide") {
                ...
                if (avcC) {
                    ...
                    sps =
                        avcC.sequence &&
                        avcC.sequence.map((item) => Number(`0x${item}`));
                    pps =
                        avcC.pps && avcC.pps.map((item) => Number(`0x${item}`));
            ...
            if (hdlr.handleType === "soun") {
                ...
                if (ESDescriptor && ESDescriptor.EScode) {
                    decoderConfig = ESDescriptor.EScode.map((item) =>
                        Number(`0x${item}`)
                    );
                }
                if (mp4a) {
    ...
    get videoKeyFrames() {
        ...
        let frames = this.getSamplesByOrders(
            "video",
            stss.entries.map((item) => item - 1)
        );
    ...
    get audioKeyFrames() {
        ...
        audioIndex = videoFrames.map((item) => {
            return util.seekOrderSampleByTime(
                audioStts,
                audioScale,
                item.time.time / videoScale
            );
        });
    ...
    loadFragment(fragIndex) {
        ...
        let audioFrame = this.getSamplesByOrders(
            "audio",
        ...
        if (fragIndex < this.videoKeyFrames.length - 1) {
            ...
            let audioNextFrame = this.getSamplesByOrders(
                "audio",
    ...
    createFragment(mdatData, start, fragIndex) {
        ...
        this.bufferCache.add(fragIndex);
        {
            ...
            let _samples = self.getSamplesByOrders(
                "video",
                framesIndex[fragIndex],
                framesIndex[fragIndex + 1]
            );
        ...
        let _samples = this.getSamplesByOrders(
            "audio",
            this.audioKeyFrames[fragIndex].order,
            this.audioKeyFrames[fragIndex + 1]
                ? this.audioKeyFrames[fragIndex + 1].order
                : undefined
        );
        let samples = _samples.map((item, idx) => {
    ...
    loadFragmentForCut(fragIndexStart, fragIndexEnd) {
        ...
        let audioStartFrame = this.getSamplesByOrders(
            "audio",
            this.audioKeyFrames[fragIndexStart].order,
            0
        );
        start = Math.min(videoStartFrame.offset, audioStartFrame.offset);
        let videoEndFrame = this.videoKeyFrames[fragIndexEnd];
        let audioEndFrame = this.getSamplesByOrders(
            "audio",
        ...
        return this.getData(
            ...
            return self.createFragmentForCut(
                new Uint8Array(dat),
                start,
                fragIndexStart,
                end,
    ...
    createFragmentForCut(mdatData, start, fragIndexStart, end, fragIndexEnd) {
        ...
        let resBuffers = [];
        {
            ...
            let _samples = self.getSamplesByOrders(
                "video",
                framesIndex[fragIndexStart],
                framesIndex[fragIndexEnd]
            );
            let samples = _samples.map((item, idx) => {
                return {
                    size: item.size,
        ...
        let _samples = this.getSamplesByOrders(
            "audio",
            this.audioKeyFrames[fragIndexStart].order,
            this.audioKeyFrames[fragIndexEnd]
                ? this.audioKeyFrames[fragIndexEnd].order
                : undefined
        );
        let samples = _samples.map((item, idx) => {
